...
// Blocos de conteúdo condensados em accordion (apenas um aberto por vez) para reduzir a extensão
// visual da seção sem cortar texto — ver DashedLink/AccordionItem já usados no FAQ.
const efficiencyAccordionData = [
  {
    id: 'importa',
  ...
  {
    id: 'evolucao',
    title: 'Nosso Compromisso com a Evolução Contínua',
    content: (
      <>
  ...
  {
    id: 'decadas',
    title: 'Um Compromisso com as Próximas Décadas',
    content: (
      <div className="space-y-3 text-neutral-600 text-sm md:text-base leading-relaxed">
        <p>
  ...
  {
    id: 'investimentos',
    title: 'Nossos Investimentos em Eficiência',
    content: (
 ...
 * - Blocos "Por que importa/Compromissos/Décadas/Investimentos" em accordion (um aberto por vez) para
 *   comprimir a leitura inicial sem perder conteúdo.
 */
export default function Sustainability({
  factoryImage = factoryImageUrl,
}) {
  // Accordion de abertura única: abrir um item fecha o anterior; nenhum aberto por padrão.
  ...
  return (
    <section className="relative w-full overflow-hidden bg-surface-darkest py-20 md:py-28 px-4 sm:px-6 lg:px-12 select-none min-h-[760px] flex items-center">
      ...
      <img
        src={factoryImage}
        alt="Vista aérea do telhado da Sydorak com sistema fotovoltaico"
      ...
      <div className="relative z-20 max-w-7xl mx-auto w-full flex justify-end">
        ...
        >
          ...
          {/* Blocos "Por que importa / Compromissos / Décadas / Investimentos" em accordion — apenas um
              aberto por vez, todos fechados por padrão, para reduzir a leitura inicial sem cortar texto. */}
          <div className="pt-2 flex flex-col gap-3">
            {efficiencyAccordionData.map((item) => (
              <AccordionItem
                key={item.id}
                title={item.title}
