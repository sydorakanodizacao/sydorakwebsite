 ...
 * 
 * DESIGN.md:
 * - Fundo bg-surface/60 com borda hairline, ícone oficial WhatsApp e botão verde proeminente posicionado
 *   abaixo da headline para encaixe perfeito dentro da box em todas as resoluções.
 * 
 * @param {Object} props
 * @param {string} props.title - Título do card.
 * @param {string} [props.description] - Descrição opcional do card.
 * @param {string} [props.buttonText] - Texto do botão CTA (padrão: "Conversar agora").
 * @param {string} [props.linkText] - Compatibilidade retroativa.
...
export default function WhatsappCard({
  ...
  const displayText = buttonText || linkText || 'Conversar agora'
  ...
  return (
    <a
      ...
      className={cn(
        'group flex flex-col gap-4 p-5 sm:p-6 w-full rounded-2xl border border-hairline bg-surface/60 hover:bg-surface hover:border-[#25D366]/50 transition-all duration-300 select-none cursor-pointer shadow-sm',
        className
      )}
      {...props}
    >
      {/* Topo: Ícone WhatsApp + Headline */}
      <div className="flex items-center gap-3.5 w-full">
        <div className="flex-shrink-0 size-8 sm:size-9 md:size-10">
          <svg
            width="100%"
            height="100%"
        ...
        <div className="flex flex-col min-w-0">
          <h4 className="text-base sm:text-lg md:text-xl text-ink group-hover:text-[#1FA855] transition-colors duration-300 font-bold leading-tight tracking-tight">
            {title}
          </h4>
          {description && (
      ...
      {/* Botão Verde WhatsApp Posicionado Abaixo da Headline */}
      <div className="inline-flex items-center justify-center gap-2 bg-[#25D366] group-hover:bg-[#20bd5a] text-white font-semibold text-sm md:text-base px-6 py-3 rounded-xl shadow-sm group-hover:shadow transition-all duration-300 w-full">
