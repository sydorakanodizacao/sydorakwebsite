 ...
 * 
 * DESIGN.md:
 * - Fundo bg-surface/60 com borda hairline, ícone oficial WhatsApp e botão verde proeminente.
 * 
 * @param {Object} props
...
export default function WhatsappCard({
  ...
  return (
    <a
      ...
      className={cn(
        'group flex flex-col gap-4 p-5 sm:p-6 w-full rounded-2xl border border-hairline bg-surface/50 hover:bg-surface hover:border-[#25D366]/40 transition-all duration-300 select-none cursor-pointer shadow-sm',
        className
      )}
      {...props}
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            <h4 className="text-[15px] sm:text-lg md:text-xl text-ink group-hover:text-[#1FA855] transition-colors duration-300 font-bold leading-tight whitespace-nowrap tracking-tight">
              {title}
            </h4>
            {description && (
        ...
        <div className="inline-flex items-center justify-center gap-2 bg-[#25D366] group-hover:bg-[#20bd5a] text-white font-semibold text-sm md:text-base px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl shadow-sm group-hover:shadow transition-all duration-300 shrink-0 w-full sm:w-auto">
